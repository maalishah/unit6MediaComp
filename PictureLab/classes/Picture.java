...
public class Picture extends SimplePicture 
{
    ...
    public void collage()
    {
        ...
        Pixel[][] rcfArray = rcf.getPixels2D();
        Pixel[][] rcfArray2 = rcf.getPixels2D();
        Pixel[][] rcfArray3 = rcf.getPixels2D();
        Pixel[][] rcfArray4 = rcf.getPixels2D();
        //rcf.mirrorVerticalRightToLeft();
        rcf.mirrorDiagonal();
        
        rcf.green();
        for(int row = 0; row < rcfArray.length; row++)
        {
            for(int col = 0; col < rcfArray[0].length; col++)
            {
                Color color = rcfArray[row][col].getColor();
                pixels[row][col].setColor(color);
                
            }
        }
        rcf.grayscale();
        for(int row = 0; row < rcfArray2.length; row++)
        {
            for(int col = 0; col < rcfArray2[0].length; col++)
            {
                Color color = rcfArray2[row][col].getColor();
                pixels[row][col+ 600].setColor(color);

            }
        }
        rcf.negate();
        for(int row = 0; row < rcfArray3.length; row++)
        {
            for(int col = 0; col < rcfArray3[0].length; col++)
            {
                Color color = rcfArray3[row][col].getColor();
                pixels[row + 323][col].setColor(color);

            }
        }
        rcf.blue();
        for(int row = 0; row < rcfArray4.length; row++)
        {
            for(int col = 0; col < rcfArray4[0].length; col++)
            {
                Color color = rcfArray4[row][col].getColor();
                pixels[row +323][col + 600].setColor(color);
